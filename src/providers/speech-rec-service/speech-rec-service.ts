...
export class SpeechRecServiceProvider {
   ...
   * @param mots ensemble des mots à traiter
   */
  public reconnaissanceAction(mots : string[]) : {reconnu :boolean, success : boolean, message_error : string}{
    ...
    resultat_action = {action_success : false, message_error : "L'action n'a pas pu être réalisée"};
    ...
    return {reconnu : phrase_reconnue, success : resultat_action.action_success, message_error : resultat_action.message_error};
  }
...
*******************************************/
   ...
   */
  private does_todo_existed(name : string, uuidList : string) : {todo : ITodoItem , success : boolean } {
...
*******************************************/
   ...
   * @param mots ensemble des mots entendus par le micro
   */
  private creerListe(mots : string[]) : {action_success : boolean, message_error : string} {
    ...
    if(liste.exist){
      ...
      this.speechSynthService.synthText("Liste " + name + " créée.");
      action_success = true;
    }
 ...
 * @param name nom de la liste à ajouter
 */
private addNewList(name : string) : void {
   ...
   * @param mots ensemble de mots entendus par le micro
   */
  private afficherListe(mots : string[]) : {action_success : boolean, message_error : string} {
    let action_success = false;
    let message_error = "L'action n'a pas pu être réalisée";
    ...
    if(liste_search.exist){
      ...
      this.evtCtrl.getNavRequestSubject().next({page:'TodoListPage', data:{uuid: liste_search.list.uuid}});
    }
    else{
   ...
   * @param mots ensemble de mots entendus par le micro
   */
  private updateListe(mots : string[]) : {action_success : boolean, message_error : string} {
    let action_success = false;
    let message_error = "L'action n'a pas pu être réalisée";
    const nomListe : string = this.getNameList(mots);
    const liste_search = this.does_list_exist(nomListe);
    ...
    if(liste_search.exist){
      ...
      this.evtCtrl.getNavRequestSubject().next({page:'ListEditPage', data:{uuid: liste_search.list}});
    }
    else{
      message_error = "La liste " + nomListe + "n'a pas été trouvée";
    }
   ...
   * @param mots ensemble des mots entendus par le micro
   */
  private supprimerListe(mots : string[]) : {action_success : boolean, message_error : string} {
    let action_success = false;
    let message_error = "L'action n'a pas pu être réalisée";
...
*******************************************/
   ...
   * @param mots ensemble de mots entendus par le micro
   */
  private creerTache(mots : string[]) : {action_success : boolean, message_error : string} {
    let action_success = false;
    let message_error = "L'action n'a pas pu être réalisée";
    ...
    if(list_search.exist && list_search.list.uuid != null){
      ...
      if(action_success){
        this.speechSynthService.synthText("Tâche " + nameTodo + " a été ajoutée dans la liste " + list_search.list.name);
        this.evtCtrl.getNavRequestSubject().next({page:'TodoListPage', data:{uuid: list_search.list.uuid}});
      }
      else{
   ...
   * @param mots ensemble de mots entendus par le micro
   */
  private updateTache(mots : string[]) : {action_success : boolean, message_error : string} {
    let action_success = false;
    ...
    if(list_found.exist && list_found.list.uuid != null){
      ...
      if(todo_found.success){
        this.evtCtrl.getNavRequestSubject().next({page:'TodoEditPage', data:{todoRef: todo_found.todo.ref}});
        this.speechSynthService.synthText("Vous pouvez maintenant modifier la tâche " + nameTodo + " de la liste " + nameList);
        action_success = true;
      }
      else{
   ...
   * @param mots ensemble des mots entendus par le micro
   */
  private supprimerTache(mots : string[]) : {action_success : boolean, message_error : string} {
    let action_success = false;
    let message_error = "L'action n'a pas pu être réalisée";
    ...
    if(liste_search.exist && liste_search.list.uuid != null){
      ...
      if(todo_search.success && todo_search.todo.ref != null && todo_search.todo.uuid != null){
          ...
          this.evtCtrl.getNavRequestSubject().next({page:'TodoListPage', data:{uuid: liste_search.list}});
          action_success = true;    
      }
      else{
